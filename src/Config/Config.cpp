...
const unsigned long FEEDING_START_DELAY = 400;         // 400ms start delay before feeding
const unsigned long FEED_CYLINDER_EXTEND_TIME = 2200; // 2 seconds feed cylinder extended (pushing wood)
const unsigned long FEED_CYLINDER_RETRACT_TIME = 10;  // 10ms retract time to feed wood

// Flipping state timing
const unsigned long FLIP_SERVO_MOVE_DELAY = 2200;      // 2200ms wait for servo to reach position
...
const int FLIP_SERVO_FLIP_POSITION = 110;             // 180 degrees servo position for flipping 