...
//! ************************************************************************
//! STEP 2: EXTEND FEED CYLINDER WITH HIGH SIGNAL (2 seconds)
//! ************************************************************************
void executeExtendCylinder() {
    static bool stepStarted = false;
    
    if (!stepStarted) {
        Serial.println("FEEDING STEP 2: Extending feed cylinder with LOW signal (2 seconds)");
        
        // Send LOW signal to extend cylinder (LOW extends)
...
void executeFeedingRetractCylinder() {
    ...
    if (!stepStarted) {
        Serial.println("FEEDING STEP 3: Retracting feed cylinder to feed wood (50ms)");
        
        // Send HIGH signal to retract cylinder (HIGH retracts and feeds wood)
